...
async function ensureUserDoc(user) {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      uid: user.uid,
      displayName: user.displayName || '운동인',
      email: user.email,
...
function showApp(user) {
  ...
  if (typeof updateRPGUI === 'function') { updateRPGUI(false); updateSessionUI(); }
}
...
function addExp(type) {
  ...
  lifetimeReps[type]++;
  workoutDays.add(new Date().toISOString().split('T')[0]);
  let leveledUp=false;
...
//  기록 페이지 렌더링
// ══════════════════════════════════════
const DUMMY_WEEK  = [45,80,20,60,95,30,0];
const DUMMY_MONTH = [12,8,15,10,18,14,9,20,6,11,17,13,8,22,5,16,19,7,14,11,25,9,18,12,6,20,15,8,0,0,0];
let chartMode = 'week';
...
function renderBarChart() {
  const data = chartMode==='week'
    ? {vals:DUMMY_WEEK, labels:['월','화','수','목','금','토','일']}
    : {vals:DUMMY_MONTH.slice(0,new Date().getDate()), labels:Array.from({length:new Date().getDate()},(_,i)=>i+1+'')};
  const max = Math.max(...data.vals,1);
  const today = chartMode==='week' ? (new Date().getDay()+6)%7 : new Date().getDate()-1;
  const chart = document.getElementById('bar-chart');
  chart.innerHTML = data.vals.map((v,i)=>`
    <div class="bar-col">
      <div class="bar-val">${v||''}</div>
      <div class="bar-body ${i===today?'highlight':''}" style="height:${Math.max(4,(v/max)*72)}px"></div>
      <div class="bar-day">${data.labels[i]}</div>
    </div>`).join('');
}
...
function renderRecord(){
  document.getElementById('r-squat-total').textContent=lifetimeReps.squat||120;
  document.getElementById('r-pushup-total').textContent=lifetimeReps.pushup||85;
  document.getElementById('r-pullup-total').textContent=lifetimeReps.pullup||40;
  renderBarChart(); renderCal();
}
...
// ══════════════════════════════════════
const DUMMY_RANK=[
  {name:'김철수',id:'@chulsoo',lv:18,exp:8420,avatar:'🦁',color:'#f97316'},
  {name:'이민지',id:'@minji_fit',lv:15,exp:6200,avatar:'🐯',color:'#a855f7'},
  {name:'박준혁',id:'@junhyuk99',lv:14,exp:5800,avatar:'🦊',color:'#22c55e'},
  {name:'최수현',id:'@soohyun_ex',lv:12,exp:4900,avatar:'🐺',color:'#4A8FFF'},
  {name:'정다은',id:'@daeun_workout',lv:11,exp:4200,avatar:'🦋',color:'#ec4899'},
  {name:'강민준',id:'@minjun_gym',lv:10,exp:3800,avatar:'🐻',color:'#f59e0b'},
  {name:'윤서연',id:'@seoyeon',lv:9,exp:3200,avatar:'🦅',color:'#14b8a6'},
  {name:'오태양',id:'@taeyang_fit',lv:8,exp:2700,avatar:'🐉',color:'#ef4444'},
  {name:'한지우',id:'@jiwoo_health',lv:7,exp:2100,avatar:'🦄',color:'#8b5cf6'},
  {name:'송은혁',id:'@eunhyuk_s',lv:6,exp:1600,avatar:'🐸',color:'#10b981'},
];
const medals=['🥇','🥈','🥉'];

function renderRank(){
  document.getElementById('my-rank-exp').textContent=totalExp+' EXP';
  const list=document.getElementById('rank-list');
  list.innerHTML=DUMMY_RANK.map((u,i)=>`
    <div class="rank-item">
      <div class="rank-pos ${i===0?'gold':i===1?'silver':i===2?'bronze':''}">${i<3?medals[i]:i+1}</div>
      <div class="rank-avatar" style="background:${u.color}22">${u.avatar}</div>
      <div class="rank-info">
        <div class="rank-name">${u.name}</div>
        <div class="rank-detail">${u.id} · LV.${u.lv}</div>
      </div>
      <div class="rank-exp-val">${u.exp.toLocaleString()}</div>
    </div>`).join('');
}
...
// ══════════════════════════════════════
const DUMMY_FRIENDS=[
  {name:'김철수',id:'@chulsoo',lv:18,avatar:'🦁',color:'#f97316',status:'오늘 스쿼트 45개',added:true},
  {name:'이민지',id:'@minji_fit',lv:15,avatar:'🐯',color:'#a855f7',status:'LV.15 달성!',added:true},
  {name:'박준혁',id:'@junhyuk99',lv:14,avatar:'🦊',color:'#22c55e',status:'3일 연속 운동 중',added:true},
];
const DUMMY_CHALLENGES=[
  {name:'김철수',avatar:'🦁',myScore:32,theirScore:45},
  {name:'이민지',avatar:'🐯',myScore:28,theirScore:20},
];

function renderSocial(){
  const fl=document.getElementById('friend-list');
  fl.innerHTML=DUMMY_FRIENDS.map(f=>`
    <div class="friend-item">
      <div class="friend-avatar" style="background:${f.color}22">${f.avatar}</div>
      <div class="friend-info">
        <div class="friend-name">${f.name}</div>
        <div class="friend-status">${f.status}</div>
      </div>
      <div class="friend-lv">LV.${f.lv}</div>
    </div>`).join('');
  document.getElementById('friend-count').textContent=DUMMY_FRIENDS.length;

  const cl=document.getElementById('challenge-list');
  cl.innerHTML=DUMMY_CHALLENGES.map(c=>{
    const tot=Math.max(c.myScore+c.theirScore,1);
    const myPct=Math.round((c.myScore/tot)*100);
    const win=c.myScore>c.theirScore;
    return `<div style="margin-bottom:10px">
      <div class="challenge-row">
        <div class="challenge-who">${c.avatar} ${c.name}</div>
        <div class="challenge-score" style="color:${win?'#22c55e':'#ef4444'}">${c.myScore} vs ${c.theirScore}</div>
      </div>
      <div class="vs-bar"><div class="vs-fill" style="width:${myPct}%;background:${win?'#22c55e':c.theirScore>c.myScore?'#ef4444':'var(--blue)'}"></div></div>
    </div>`;
  }).join('');
}
...
function searchFriend(){
  ...
  if(!q) return;
  // 실제로는 Firebase에서 검색
  alert('Firebase 연결 후 실제 검색이 됩니다!\n검색어: '+q);
}
