...
.badge-item.unlocked{border-color:rgba(245,200,66,.4);background:rgba(245,200,66,.06);}
.badge-category-title{font-size:11px;font-weight:700;color:var(–sub);letter-spacing:.5px;text-transform:uppercase;padding:14px 16px 6px;}
...
<div class="page" id="page-profile">
  ...
  <div class="section" style="margin-top:20px"><div class="section-title">획득 뱃지 <span id="badge-count" style="color:var(--blue);font-size:11px"></span></div></div>

  <!-- 레벨 뱃지 -->

  <div class="badge-category-title">🎖️ 레벨</div>
  <div class="badge-grid" id="badges-level"></div>

  <!-- 스쿼트 뱃지 -->

  <div class="badge-category-title">🦵 스쿼트</div>
  <div class="badge-grid" id="badges-squat"></div>

  <!-- 푸시업 뱃지 -->

  <div class="badge-category-title">💪 푸시업</div>
  <div class="badge-grid" id="badges-pushup"></div>

  <!-- 풀업 뱃지 -->

  <div class="badge-category-title">🏋️ 풀업</div>
  <div class="badge-grid" id="badges-pullup"></div>

  <!-- 콤보 뱃지 -->

  <div class="badge-category-title">⚡ 콤보</div>
  <div class="badge-grid" id="badges-combo"></div>

  <!-- 특별 뱃지 -->

  <div class="badge-category-title">✨ 특별</div>
  <div class="badge-grid" id="badges-special"></div>
...
const BADGE_DEFS = {
  level: [
    { icon:'🥉', name:'LV.10',  cond: () => currentLevel >= 10  },
    { icon:'🥈', name:'LV.20',  cond: () => currentLevel >= 20  },
    { icon:'🥇', name:'LV.30',  cond: () => currentLevel >= 30  },
    { icon:'💫', name:'LV.40',  cond: () => currentLevel >= 40  },
    { icon:'👑', name:'LV.50',  cond: () => currentLevel >= 50  },
    { icon:'💎', name:'LV.100', cond: () => currentLevel >= 100 },
  ],
  squat: [
    { icon:'🦵', name:'스쿼트 50',   cond: () => lifetimeReps.squat >= 50   },
    { icon:'🦵', name:'스쿼트 100',  cond: () => lifetimeReps.squat >= 100  },
    { icon:'🦵', name:'스쿼트 200',  cond: () => lifetimeReps.squat >= 200  },
    { icon:'🦵', name:'스쿼트 300',  cond: () => lifetimeReps.squat >= 300  },
    { icon:'🦵', name:'스쿼트 500',  cond: () => lifetimeReps.squat >= 500  },
    { icon:'🔱', name:'스쿼트 1000', cond: () => lifetimeReps.squat >= 1000 },
  ],
  pushup: [
    { icon:'💪', name:'푸시업 50',   cond: () => lifetimeReps.pushup >= 50   },
    { icon:'💪', name:'푸시업 100',  cond: () => lifetimeReps.pushup >= 100  },
    { icon:'💪', name:'푸시업 200',  cond: () => lifetimeReps.pushup >= 200  },
    { icon:'💪', name:'푸시업 300',  cond: () => lifetimeReps.pushup >= 300  },
    { icon:'💪', name:'푸시업 500',  cond: () => lifetimeReps.pushup >= 500  },
    { icon:'🔱', name:'푸시업 1000', cond: () => lifetimeReps.pushup >= 1000 },
  ],
  pullup: [
    { icon:'🏋️', name:'풀업 50',   cond: () => lifetimeReps.pullup >= 50   },
    { icon:'🏋️', name:'풀업 100',  cond: () => lifetimeReps.pullup >= 100  },
    { icon:'🏋️', name:'풀업 200',  cond: () => lifetimeReps.pullup >= 200  },
    { icon:'🏋️', name:'풀업 300',  cond: () => lifetimeReps.pullup >= 300  },
    { icon:'🏋️', name:'풀업 500',  cond: () => lifetimeReps.pullup >= 500  },
    { icon:'🔱', name:'풀업 1000', cond: () => lifetimeReps.pullup >= 1000 },
  ],
  combo: [
    { icon:'⚡',  name:'콤보 10x',  cond: () => (window._maxCombo||0) >= 10  },
    { icon:'⚡',  name:'콤보 20x',  cond: () => (window._maxCombo||0) >= 20  },
    { icon:'🌩️', name:'콤보 30x',  cond: () => (window._maxCombo||0) >= 30  },
    { icon:'🌩️', name:'콤보 40x',  cond: () => (window._maxCombo||0) >= 40  },
    { icon:'🌪️', name:'콤보 50x',  cond: () => (window._maxCombo||0) >= 50  },
    { icon:'☄️',  name:'콤보 100x', cond: () => (window._maxCombo||0) >= 100 },
  ],
  special: [
    { icon:'🔥', name:'첫 운동',   cond: () => (lifetimeReps.squat+lifetimeReps.pushup+lifetimeReps.pullup) >= 1 },
    { icon:'📅', name:'7일 연속',  cond: () => streak >= 7   },
    { icon:'🗓️', name:'30일 연속', cond: () => streak >= 30  },
    { icon:'🏅', name:'100일 연속',cond: () => streak >= 100 },
    { icon:'👾', name:'Legend',    cond: () => currentLevel >= 25 },
    { icon:'🌟', name:'Master',    cond: () => currentLevel >= 30 },
  ],
};

function renderBadgeGroup(groupId, badges) {
  const el = document.getElementById('badges-' + groupId);
  if (!el) return;
...
function renderProfile(){
  ...
  document.getElementById('p-total-exp').textContent = totalExp.toLocaleString();
  document.getElementById('p-streak').textContent = streak || calcStreak(workoutDays);

  // 최대 콤보 갱신
  if (combo > (window._maxCombo||0)) window._maxCombo = combo;

  // 뱃지 렌더
  Object.keys(BADGE_DEFS).forEach(key => renderBadgeGroup(key, BADGE_DEFS[key]));

  // 획득 수 표시
  const all = Object.values(BADGE_DEFS).flat();
  const got = all.filter(b => b.cond()).length;
  const el = document.getElementById('badge-count');
  if (el) el.textContent = `${got}/${all.length}`;
}
